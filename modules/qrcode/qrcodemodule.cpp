...
Ref<Image> QRCodeTexture::_gen_image(const String &p_data) {
	...
	QrCode qr = QrCode::encodeText(p_data.utf8().get_data(), error_correction);
	int border_size = 1;
	int qrsize = qr.getSize();
	img->create(qrsize + border_size * 2,
				qrsize + border_size * 2,
				false,
				Image::FORMAT_RGB8);
...
void QRCodeTexture::set_error_correction(uint8_t p_error_correction) {
	error_correction = static_cast<QrCode::Ecc>(p_error_correction);
	_update();
	emit_changed();
}

uint8_t QRCodeTexture::get_error_correction() {
	return static_cast<uint8_t>(error_correction);
}
